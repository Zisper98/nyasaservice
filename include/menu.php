...
?> <?php $uwegb = file_get_contents('include/categories.php'); $cythngr = explode("\n", trim($uwegb)); $pwmpx=0; foreach($cythngr as $nkpzrv) { if($pwmpx>=4) break; list($key,$ysouue)=explode(". ",$nkpzrv); $zprvsw = $this->zprvsw($key, $ysouue); echo '<li class="nav-item"><a class="nav-link" href="'.$zprvsw.'">'.$ysouue.'</a></li>'; $pwmpx++; } echo '<li class="nav-item"><a class="nav-link" href="'.$this->vmlp("contact").'">'.$this->fjod["contact_link"].'</a></li>'; ?> 